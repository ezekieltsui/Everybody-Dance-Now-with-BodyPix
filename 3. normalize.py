...
target_img = cv2.imread('./data/target/final_label_images/00001.png')[:,:,0]
# target_img_rgb = cv2.imread('./data/target/label_images/00001.png')
source_img = cv2.imread('./data/source/final_label_images/00788.png')[:,:,0]
# source_img_rgb = cv2.imread('./data/source/label_images/00788.png')

path = './data/source/final_label_images/'
save_dir = Path('./data/source/')
output = save_dir.joinpath('final_label_images_norm')
output.mkdir(exist_ok=True)
# head_dir = save_dir.joinpath('head_image_norm')
# head_dir.mkdir(exist_ok=True)
# pose_dir = Path('./data/source/head_cords.npy')
...
for img_idx in tqdm(range(len(os.listdir(path)))):
    ...
    new_source = source_pad[
                #  (source_head_rs_x - target_head_x):(source_head_rs_x + (target_img.shape[0] - target_head_x)),
                 int((source_pad.shape[1] - target_img.shape[1])/2):int((source_pad.shape[1]-(source_pad.shape[1] -
                                                                                              target_img.shape[1])/2))          #1
                 ]
